...
func LoginWithLinuxDo(r *http.Request, code string, state string) (model.AuthSession, string, error) {
	...
	if !ok {
		...
		registerIP := clientIPFromRequest(r)
		if _, exists, err := repository.GetUserByRegisterIP(registerIP); err != nil || exists {
			if err != nil {
				return model.AuthSession{}, redirect, err
			}
			return model.AuthSession{}, redirect, safeMessageError{message: "同一 IP 只允许注册一个账号"}
		}
		user = model.User{
			ID:          newID("user"),
...
func AdjustUserCredits(id string, credits int) (model.User, error) {
	...
	user, ok, err = repository.SetUserCreditsWithLog(id, credits, nowText, log)
	if err != nil {
		return user, err
	}
	if !ok {
...
func saveRegisterBonusCreditLog(user model.User) error {
