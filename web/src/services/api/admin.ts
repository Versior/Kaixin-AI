...
export type AdminGenerationLogListResponse = {
...
export async function fetchAdminGenerationLogs(token: string, query: AdminAssetQuery = {}) {
    return apiGet<AdminGenerationLogListResponse>("/api/admin/generation-logs", compactApiParams(query), token);
}

export async function deleteAdminGenerationLog(token: string, id: string) {
