...
本文档说明当前灵感在前端本地保存的数据结构、图片文件的存储和清理方式，以及后续接入后端存储时建议保持的兼容边界。

## 当前存储位置

当前灵感项目主要保存在浏览器本地：

- 灵感项目 JSON：`localForage`，数据库名 `infinite-canvas`，storeName `app_state`，key 为 `infinite-canvas:canvas_store`。
...
灵感 JSON 不直接长期保存大体积 base64 图片或视频。图片节点、视频节点、助手图片和素材媒体只保存展示 URL、`storageKey` 和元信息，真实 Blob 通过 `storageKey` 读取。

## 灵感项目结构

每个灵感项目是一个 `CanvasProject`：
...
- `nodes`：灵感节点列表。
- `connections`：节点连线列表。
- `chatSessions`：右侧灵感助手会话。
- `activeChatId`：当前选中的助手会话 ID。
- `backgroundMode`：灵感背景模式。
...
- `title`：节点标题。
- `position`：灵感世界坐标，不是屏幕坐标。
- `width` / `height`：灵感世界坐标下的节点尺寸。
- `metadata`：节点内容和业务状态。
...
1. 删除节点、清空灵感、删除灵感、删除素材、删除助手会话时，会触发 `cleanupImages`。
2. `cleanupImages` 会收集当前仍被灵感项目、素材和额外传入数据引用的所有 `storageKey`。
3. `cleanupUnusedImages` 遍历 `image_files` 中的全部图片。
4. 不在引用集合里的图片会被删除。
5. 删除时会同时 `URL.revokeObjectURL`，并从内存缓存 `objectUrls` 移除。

这套方式可以避免同一张图片被灵感、素材或助手同时引用时误删。
...
后续接入后端时，建议保持“灵感 JSON”和“图片文件”分离：

- 灵感表保存项目元信息和灵感 JSON。
- 文件表保存图片文件、访问 URL、哈希、大小、MIME、宽高、归属用户等信息。
- 灵感节点中继续保存轻量图片引用，不把图片二进制或 base64 写进灵感 JSON。
...
- 删除节点只删除灵感 JSON 引用，不直接删除后端文件。
- 后端文件删除应按引用计数或定期扫描未引用文件处理。
- 保存到“我的素材”的图片，即使原灵感节点删除，也应继续保留文件引用。
- 删除灵感、删除素材、删除助手会话后，再由后端清理任务判断文件是否无人引用。

建议同步流程：

1. 前端保存灵感 JSON 时，保持节点 ID、连线 ID、`storageKey/fileId` 不变。
...
- 不要把新生成的大图直接长期写入灵感 JSON。
- 新增图片来源时统一走 `uploadImage` 或未来的文件上传服务。
- 新增图片引用字段时，应保留 `storageKey` 兼容旧本地数据。
- 新增清理入口时，要把仍需保留的灵感、素材、助手数据传给 `cleanupUnusedImages`。
